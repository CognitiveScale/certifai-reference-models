...
class Encoder:
    ...
    def infer_from_dataset(self):
        ...
        self.num_col__ = num_col_
        self.cat_col__ = cat_col_
    ...
    def transform(self, X):
        ...
        num_rows = X.shape[0]
        all_column_names = [
            name for i in range(len(self.data_dict__)) for name in self.data_dict__[i]
        ]
        ...
        for idx, header in enumerate(X):
            ...
            if idx in self.cat_col__:
                encoded_col = self.encodeColumn(X[header], self.data_dict__[idx])
                encoded_col = encoded_col.reshape(
                    (X.shape[0], len(self.data_dict__[idx]))
                )
            if idx in self.num_col__:
                encoded_col = (
                    X[header].values.astype(float) - self.mean_[idx]
                ) / self.scale_[idx]
