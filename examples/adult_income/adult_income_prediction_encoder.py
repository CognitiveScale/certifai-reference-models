...
income_dict, num_col, cat_col = load_adult_income_dict(X)
...
class CategoricalEncoder:
    ...
    def transform(self, X):
        ...
        num_rows = X.shape[0]
        all_column_names = [name for i in range(len(income_dict)) for name in income_dict[i]]
        ...
        for idx, header in enumerate(X):
            ...
            if idx in cat_col:
                encoded_col = self.encodeColumn(X[header], income_dict[idx])
                encoded_col = encoded_col.reshape((X.shape[0], len(income_dict[idx])))
            if idx in num_col:
            ...
            feature_names.extend(sorted(income_dict[idx]))
