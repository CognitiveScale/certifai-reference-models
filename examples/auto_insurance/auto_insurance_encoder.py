...
COLUMNS = ["State Code", "Claim Amount", "Coverage", "Education", "EmploymentStatus", "Gender", "Income", "Location Code", "Marital Status", "Monthly Premium Auto", "Months Since Last Claim", "Months Since Policy Inception", "Number of Open Complaints", "Number of Policies", "Policy", "Claim Reason", "Sales Channel", "Vehicle Class", "Vehicle Size"] # "Total Claim Amount"

def get_columns_dict():
    claims_dict = {'State Code': ['KS', 'NE', 'OK', 'MO', 'IA'],
                   'Claim Amount': ['Amount'],
                   'Coverage': ['Basic', 'Extended', 'Premium'],
                   'Education': ['High School or Below', 'College', 'Bachelor', 'Master', 'Doctor'],
                   'EmploymentStatus': ['Unemployed', 'Employed', 'Medical Leave', 'Disabled', 'Retired'],
                   'Gender': ['M', 'F'],
                   'Income': ['Amount'],
                   ...
                   'Policy': ['Corporate L1', 'Corporate L2', 'Corporate L3', 'Personal L1',
                              'Personal L2', 'Personal L3', 'Special L1', 'Special L2', 'Special L3'],
...
def get_expanded_cols(df_csv, claims_dict):
    ...
    for header in claims_dict:
        if isinstance(df_csv[header].iloc[0], (np.int, np.int64)) or \
                isinstance(df_csv[header].iloc[0], (np.float, np.float64)):
...
class CategoricalEncoder():
    ...
    def get_num_cat_index(self, df):
        ...
        for idx, header in enumerate(df.columns):
            if isinstance(df.iloc[0,idx], (np.int, np.int64)) or isinstance(df.iloc[0, idx], (np.float, np.float64)):
    ...
    def fit(self, X):
        ...
        num_col, cat_col = self.get_num_cat_index(X) # Determine which features are num/cat
    ...
    def transform(self, X, target = False):
        ...
        for idx,header in enumerate(X):
            if header in cat_col: # One-hot encoding if categorical
                ...
                self.cat_dict[header] = np.linspace(col_idx, col_idx+len_col-1, len_col,dtype = np.int16)
    ...
    def inverse_transform(self, X):
        ...
        for col_name, categories in columns_dict.items():
            ...
            if num_categories == 1:  # Numeric
                ...
                unnorm_values = X.iloc[:, ohe_col_start_idx].apply(lambda x: x * (max_val - min_val) + min_val)
