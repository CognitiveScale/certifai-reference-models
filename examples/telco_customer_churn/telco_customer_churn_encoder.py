...
# Make sure the categorical columns are all of dtype object
def load_telco_customer_churn_dict(df):
    df_dict = {}
    num_col_ = []
    cat_col_ = []
...
churn_dict, num_col, cat_col = load_telco_customer_churn_dict(X)
...
class CategoricalEncoder:
    ...
    def transform(self, X):
        ...
        num_rows = X.shape[0]
        all_column_names = [name for i in range(len(churn_dict)) for name in churn_dict[i]]
        ...
        for idx, header in enumerate(X):
            ...
            if idx in cat_col:
                encoded_col = self.encodeColumn(X[header], churn_dict[idx])
                encoded_col = encoded_col.reshape((X.shape[0], len(churn_dict[idx])))
            if idx in num_col:
            ...
            feature_names.extend(sorted(churn_dict[idx]))
